...
import { Coins, Mail, User, ShieldCheck, Edit3 } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex justify-between items-end">
        ...
        <Link to="/profile/edit">
          <motion.button 
            whileHover={{ y: -2, boxShadow: "0 0 15px rgba(52,211,153,0.3)" }}
            className="flex items-center gap-2 px-5 py-2.5 bg-[#0a0f0d] border border-emerald-500/50 text-emerald-400 rounded-xl hover:bg-emerald-500/10 transition-all font-semibold"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          {/* Avatar Ring with Dynamic Image */}
          <div className="w-32 h-32 rounded-full border-4 border-emerald-500/30 flex items-center justify-center bg-[#0a0f0d] shrink-0 relative shadow-[0_0_20px_rgba(52,211,153,0.2)]">
            {user?.profilePicture ? (
              <img src={user.profilePicture} alt="Profile" className="w-full h-full rounded-full object-cover relative z-10" />
            ) : (
          ...
          <div className="space-y-4 text-center md:text-left w-full">
            ...
            <div className="flex items-center justify-center md:justify-start gap-3 text-slate-400 bg-[#0a0f0d]/50 p-3 rounded-xl border border-emerald-900/30 w-fit">
